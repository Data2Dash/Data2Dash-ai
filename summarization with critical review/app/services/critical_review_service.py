...
def _clean_evidence_items(evidence: list[str], limit: int = 5) -> list[str]:
    ...
    for item in evidence:
        ...
        # Drop raw JSON fragments (key-figure dict dumps that slipped through)
...
def _score_candidate(sentence: str, keywords: tuple[str, ...], dimension: str = "") -> int:
    ...
    if dimension == "reproducibility":
        if _RESULT_NOISE.search(sentence) and not any(k in lower for k in _SETUP_KWS_REPRO):
            score -= 12
        # Penalise pure result lines (accuracy/benchmark scores) — not setup evidence
        if any(k in lower for k in ("accuracy", "bleu", "rouge", "f1", "perplexity", "benchmark score")):
...
def _format_evidence_lines(evidence: list[str]) -> list[str]:
    return [f"- {i}" for i in evidence] if evidence else ["- No explicit evidence extracted."]
...
def generate_paper_critical_profile(
    ...
    try:
        ...
        msg = str(exc).lower()
        if (
            _is_token_size_error(exc)
            or _is_rate_limit_error(exc)
            or "rate limit reached" in msg
            or "all fallback models failed" in msg
            or "upgrade your groq plan" in msg
